...
while a == True:
    ...
    listadetudo.append([dicionario['nome'], dicionario['partidas'], dicionario['gols'], dicionario['total']])
    dicionario.clear()
    listadegols.clear()
    somadegols = 0
    continuar = ' '
...
for c in range(0, len(listadetudo)):
    print(f'{c:<10}{listadetudo[c][0]:<15}{listadetudo[c][2]!s:<20s}{listadetudo[c][3]:<22}')
print('-=-' * 20)
njogador = 0
while njogador != 999:
    njogador = int(input('mostrar dados de qual jogador (999 para parar): '))
    if njogador <= len(listadetudo) - 1:
        print(f'  -- LEVANTAMENTO DO JOGADOR {listadetudo[njogador][0]}:')
        for i, v in enumerate(listadetudo[njogador][2]):
